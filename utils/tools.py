...
# 
        # logging.basicConfig(filename=self.log_path+'.log', filemode="w", format="%(asctime)s %(name)s:%(levelname)s:%(message)s", datefmt="%d-%M-%Y %H:%M:%S")
        # logging.info('test')
    ...
    def display(self, batch):
        ...
        print('\t'.join(entries))
        # input("d12432")
        # print('\t'.join(entries), file=self.log_file)
...
def accuracy(output, target, topk=(1,), top10=None):
